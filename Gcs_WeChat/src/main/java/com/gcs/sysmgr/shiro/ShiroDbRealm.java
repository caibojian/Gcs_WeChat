 ...
 * Filename:		com.gcs.sysmgr.shiro.ShiroDbRealm.java
 * Class:			ShiroDbRealm
 * Date:			2012-8-2
...
import com.gcs.sysmgr.service.impl.WechatServiceImpl;
...
//		boolean f = wechatService.validateQR(tokens.getQRcontent());
//		if(!f){
//			throw new IncorrectQRException("验证二维码码错误，请重扫描！");
//		}
		...
		CaptchaUsernamePasswordToken c_token = (CaptchaUsernamePasswordToken) authcToken;
		...
		if (user != null) {
			...
			ShiroUser shiroUser = new ShiroUser(user.getId(), user.getUsername(), user,c_token.getType());
			// 这里可以缓存认证
	...
	public static class ShiroUser implements Serializable {
		...
		public ShiroUser(Long id, String loginName, User user,String type) {
			this.id = id;
			this.loginName = loginName;
			this.user = user;
			this.type = type;
